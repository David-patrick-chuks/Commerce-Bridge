...
export class OnboardingFlow {
  private readonly onboardingWelcome = `${formatWhatsAppBold('👋 Welcome to ' + project_name + '!')}
...
`;
  ...
  private readonly welcomeMessage = `${formatWhatsAppBold('🎉 Welcome to ' + project_name + '!')}
...
Type *1* for Customer or *2* for Seller`;
  ...
  async handleOnboarding(message: any, session: UserSession, client: any): Promise<string | null> {
    ...
    if (messageText.includes('2') || messageText.includes('seller')) {
      ...
      session.currentState = 'awaiting_account_creation';
      return `Awesome! To sell on ${project_name}, you'll need to create a seller account.\n\nType *create account* or *signup* to get your registration link.`;
    }
    // Handle FAQs
...
${formatWhatsAppBold('About ' + project_name + ':')}
${project_name} helps small businesses like yours sell online easily — right inside WhatsApp! We provide digital storefronts, automated orders, and easy payment options to streamline your sales.
...
${formatWhatsAppBold('2. Is ' + project_name + ' free to use?')}
